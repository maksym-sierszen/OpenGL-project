...
void renderScene(GLFWwindow* window)
{
	...
	for (int i = 0; i < totalCrabs; ++i) {
		...
		// Określenie rzędu (pierwszy rząd: 0, drugi rząd: 1)
		int row = (i < 3) ? 0 : 1;
		// Pozycjonowanie w osi X i Z
		float xSpacing = 1.5f; // Odległość między krabami w osi X
		float xPosition = (row == 0) ? -3.0f + (i * xSpacing) : -1.5f + ((i - 3) * xSpacing); // Rozstaw w zależności od rzędu
		float zPosition = -4.0f - (row * 1.3f); // Drugi rząd jest przesunięty w osi Z o 2 jednostki
		...
		// Skalowanie (wszystkie kraby tego samego rozmiaru)
	...
	for (int i = 0; i < 3; ++i) {
		...
		// Deterministyczny obrót wokół osi Y w oparciu o indeks i
		float rotationInDegrees = 30.0f * i; // Każda muszelka obraca się o 10° więcej niż poprzednia
		float rotationInRadians = glm::radians(rotationInDegrees);

		shellMatrix = glm::translate(shellMatrix, positions[i]);
		shellMatrix = glm::rotate(shellMatrix, rotationInRadians, glm::vec3(0.0f, 1.0f, 0.0f));    // Obrót wokół osi Y
